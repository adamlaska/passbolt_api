...
/**
 * @covers \Passbolt\SsoRecover\Controller\OAuth2\OAuth2RecoverSuccessController
 */
class OAuth2RecoverSuccessControllerTest extends SsoRecoverIntegrationTestCase
{
    public function testOAuth2RecoverSuccessController_ErrorJson(): void
    {
        $this->getJson('/sso/recover/oauth2/success.json');
        $this->assertError(400, 'not supported');
    }

    public function testOAuth2RecoverSuccessController_ErrorLoggedIn(): void
    {
        $this->logInAsUser();
        $this->get('/sso/recover/oauth2/success');

        $this->assertResponseCode(403);
        $this->assertResponseContains('The user should not be logged in.');
    }

    public function testOAuth2RecoverSuccessController_ErrorNoToken(): void
    {
        $this->get('/sso/recover/oauth2/success');
    ...
    public function testOAuth2RecoverSuccessController_ErrorInvalidToken(): void
    {
        $this->get('/sso/recover/oauth2/success?token=nope');

        $this->assertResponseCode(400);
        $this->assertResponseContains('The token is required in URL parameters.');
    }

    public function testOAuth2RecoverSuccessController_ErrorTokenDeleted(): void
    {
        ...
        $this->get('/sso/recover/oauth2/success?token=' . $authToken->token);

        $this->assertResponseCode(400);
        $this->assertResponseContains('The authentication token does not exist or has been deleted.');
    }

    public function testOAuth2RecoverSuccessController_ErrorTokenExpired(): void
    {
        /** @var \Passbolt\Sso\Model\Entity\SsoAuthenticationToken $authToken */
    ...
    public function testOAuth2RecoverSuccessController_Success(): void
    {
        /** @var \Passbolt\Sso\Model\Entity\SsoAuthenticationToken $authToken */
